...
public class Picker: NSObject {
    ...
    ///   - completed: 点击完成回调
    ///   - canceled: 点击取消回调
    @discardableResult public init(datePickerOrigin: UIView, datePickerMode: UIDatePickerMode = .date, originDate: Date = Date(), completed: @escaping ((_ selectedDate: Date?)->())) {
        super.init()
